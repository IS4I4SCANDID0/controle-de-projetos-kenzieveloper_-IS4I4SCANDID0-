...
const createDeveloperInfoController = async (request: Request, response: Response): Promise<Response> => {
  ...
  const newDeveloperInfos: TDeveloperInfos = await createDeveloperInfosService(developerId, developerInfoData);
...
const updateDeveloperController = async (request: Request, response: Response): Promise<Response> => {
  const developer: TDeveloper = await developerService.updateDevelopers(request.body, request.params.id);
  return response.status(200).json(developer)
}
...
export { 
  ...
  retrieveDevelopersController, 
  updateDeveloperController,
  erasesDevelopersController
};
